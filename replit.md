...
- **Busy Areas/Hotspots**: Real-time city hotspot detection using OpenStreetMap POI density (Overpass API, free, no key) with time-of-day busyness scoring. Map view + ranked list. Free: 5mi, Premium: 25mi (filtered by route/driving distance via OSRM). Tapping a hotspot opens device maps app. Time-of-day context banner explains scoring. Real driving distances and estimated drive times via OSRM (free, no key).
